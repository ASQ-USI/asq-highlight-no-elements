var Heatmap = (function(){
  ...
  ;(function(){
    ...
    this.addRangesForHue = function(hue, ranges){
      ...
      // while(i--){
      //   for (var j = ranges[i].start.row; j <= ranges[i].end; j++){
      //     var start = 0
      //       , end = 0
      //       , k =0;

      //     // first row
      //     if(j == ranges[i].start.row){
      //       start = ranges[i].start.column;
      //       end = (ranges[i].start.row === ranges[i].end.row)
      //         ? ranges[i].end.column 
      //         : hueWeights[j];
      //     }
      //     // rows in the middle
      //     else if( j > ranges[i].start.row  && j < ranges[i].end.row){
      //       start = 0;
      //       end = textlines[j].length;
      //     }
      //     // last row
      //     else if( j > ranges[i].start.row  && j === ranges[i].end.row){
      //       start = 0;
      //       end = ranges[i].end.column;
      //     }

      //     // this is the actual update in frequency
      //     k = end;
      //     while(k-- > start){
      //       // we calculate the mean M[n] where n is the number of the terms so far incrementally
      //       // M[n] = ((n-1) * M[n-1] + newValue) / n 
      //       // which is equivalent to
      //       // M[n] = M[n-1] + (newValue - M[n-1]) / n 
      //       hueWeights[j][k] = hueWeights[j][k] + (1 - hueWeights[j][k]) / n;
      //     }
      //   }
      // }
      console.log(hueWeights)
    };
